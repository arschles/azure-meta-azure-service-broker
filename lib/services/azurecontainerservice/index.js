...
Handlers.generateAzureInstanceId = function(params) {
  return Config.name + '-' + params.parameters['acsAccountName'];
};

Handlers.provision = function(params, next) {

  log.debug('ACS/index/provision/params: %j', params);
  ...
  acsClient.initialize(params.azure);
  cp.provision(acsClient, function(err, result) {
    if (err) {
...
Handlers.poll = function(params, next) {
  ...
  log.debug('ACS/index/poll/params: %j', params);
  ...
  acsClient.initialize(params.azure);
  cp.poll(acsClient, function(err, reply, provisioningResult) {
    if (err) {
...
Handlers.deprovision = function(params, next) {
  ...
  log.debug('ACS/index/deprovision/params: %j', params);

  var cp = new cmdDeprovision(params);

  acsClient.initialize(params.azure);
  cp.deprovision(acsClient, function(err, result) {
    if (err) {
      common.handleServiceError(err, next);
    } else {
      next(null, Reply(HttpStatus.ACCEPTED), result);
...
Handlers.bind = function(params, next) {
  ...
  log.debug('ACS/index/bind/params: %j', params);
  ...
  acsClient.initialize(params.azure);
  cp.bind(acsClient, function(err, masterKey) {
    if (err) {
      common.handleServiceError(err, next);
    } else {
      var reply = {
...
Handlers.unbind = function(params, next) {
  ...
  log.debug('ACS/index/unbind/params: %j', params);
